...
import {
  ...
  ActivityIndicator, Image, Modal, FlatList, StyleSheet, SafeAreaView,
} from 'react-native';
...
export default function CreateViolationScreen() {
  ...
  const handleSubmit = useCallback(async () => {
    if (!activeRoom) return;
    if (!selectedRule) { Alert.alert('Validasi', 'Pilih aturan yang dilanggar.'); return; }
    if (selectedViolators.length === 0) { Alert.alert('Validasi', 'Pilih minimal 1 pelanggar.'); return; }
    if (photos.length === 0) { Alert.alert('Validasi', 'Unggah minimal 1 foto bukti.'); return; }
  ...
  }, [activeRoom, selectedRule, selectedViolators, photos, description, createViolation, navigation]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <Text style={styles.title} accessibilityRole="header">Buat Laporan</Text>

        {/* Rule picker */}
        <View style={styles.field}>
          <Text style={styles.label}>Aturan yang Dilanggar <Text style={styles.required}>*</Text></Text>
          <TouchableOpacity style={styles.picker} onPress={() => setRuleModalVisible(true)}
            accessibilityLabel="Pilih aturan" accessibilityRole="button">
        ...
        <View style={styles.field}>
          ...
          <TouchableOpacity style={styles.picker} onPress={() => setViolatorModalVisible(true)}
            accessibilityLabel="Pilih pelanggar" accessibilityRole="button">
            <Text style={selectedViolators.length > 0 ? styles.pickerValue : styles.pickerPlaceholder}>
        ...
        <View style={styles.field}>
          ...
          />
          <Text style={styles.charCount}>{description.length}/{MAX_DESC_CHARS}</Text>
        </View>

        {/* Photo upload */}
        <View style={styles.field}>
          <Text style={styles.label}>Foto Bukti <Text style={styles.required}>*</Text> <Text style={styles.optional}>(1–3 foto, maks {MAX_PHOTO_MB} MB)</Text></Text>
          <View style={styles.photoRow}>
            ...
            {photos.length < MAX_PHOTOS && (
              <TouchableOpacity style={styles.addPhoto} onPress={handlePickPhoto}
                accessibilityLabel="Tambah foto bukti" accessibilityRole="button">
        ...
        <TouchableOpacity
          ...
          accessibilityLabel="Kirim laporan" accessibilityRole="button">
          {isSubmitting
      ...
      {/* Rule Modal */}
      <Modal visible={ruleModalVisible} animationType="slide" transparent onRequestClose={() => setRuleModalVisible(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setRuleModalVisible(false)}>
          <View style={styles.modalSheet}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={[styles.modalItem, selectedRule?.id === item.id && styles.modalItemSelected]}
                  onPress={() => { setSelectedRule(item); setRuleModalVisible(false); }}
                  accessibilityLabel={`Pilih aturan: ${item.name}`}>
                  <Text style={styles.modalItemText}>{item.name}</Text>
                  {item.description ? <Text style={styles.modalItemSub}>{item.description}</Text> : null}
                </TouchableOpacity>
              )}
      ...
      {/* Violator Modal */}
      <Modal visible={violatorModalVisible} animationType="slide" transparent onRequestClose={() => setViolatorModalVisible(false)}>
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setViolatorModalVisible(false)}>
          <View style={styles.modalSheet}>
            ...
            <FlatList
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <TouchableOpacity style={[styles.modalItem, selected && styles.modalItemSelected]}
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                      ...
                      <View>
                        <Text style={styles.modalItemText}>{item.name}</Text>
                        {item.department ? <Text style={styles.modalItemSub}>{item.department}</Text> : null}
                      </View>
                    </View>
                  </TouchableOpacity>
                );
...
const styles = StyleSheet.create({
  ...
  picker: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff', borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 12 },
  ...
  textArea: { backgroundColor: '#fff', borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 14, color: '#111827', textAlignVertical: 'top', minHeight: 90 },
  charCount: { fontSize: 11, color: '#9CA3AF', textAlign: 'right', marginTop: 4 },
  photoRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  photoWrapper: { position: 'relative' },
  photoThumb: { width: 80, height: 80, borderRadius: 8 },
  removePhoto: { position: 'absolute', top: -6, right: -6, backgroundColor: '#EF4444', width: 20, height: 20, borderRadius: 10, alignItems: 'center', justifyContent: 'center' },
  removePhotoText: { color: '#fff', fontSize: 11, fontWeight: '700' },
  addPhoto: { width: 80, height: 80, borderRadius: 8, borderWidth: 1, borderColor: '#D1D5DB', borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center', backgroundColor: '#F9FAFB' },
  addPhotoText: { fontSize: 12, color: '#9CA3AF', textAlign: 'center' },
  submitBtn: { backgroundColor: '#3B82F6', borderRadius: 10, paddingVertical: 15, alignItems: 'center', marginTop: 8 },
  submitBtnDisabled: { backgroundColor: '#93C5FD' },
  submitBtnText: { color: '#fff', fontSize: 16, fontWeight: '600' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' },
  modalSheet: { backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 20, maxHeight: '70%', paddingBottom: 24 },
  ...
  userAvatarFallback: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#DBEAFE', alignItems: 'center', justifyContent: 'center' },
  userInitials: { fontSize: 12, fontWeight: '700', color: '#1D4ED8' },
  doneBtn: { backgroundColor: '#3B82F6', borderRadius: 8, paddingVertical: 12, alignItems: 'center', marginTop: 12 },
